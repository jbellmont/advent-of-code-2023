...
const formatAsArrayOfNumbers = (numbersList: string): number[] => {
  return numbersList.split(/\D/).filter(value => value !== '').map(value => Number(value));
}
...
const getCardCountOfRange = (scratchcards: string[], startIndex: number, length: number): number => {
  let cards = 0;

  for (let i = startIndex; i < length; i++) {
    const cardsForSingleGame = getCardCountForSingleGame(scratchcards[i]);
    const cardsFromRangeOfCards = getCardCountOfRange(scratchcards, i + 1, i + (cardsForSingleGame + 1));
...
const getSumOfScratchcards = async (
    ...
    try {
      const scratchcards = (await readFile(pathToScratchcards, { encoding: "utf-8" })).split(
        "\n"
      );

      return getCardCountOfRange(scratchcards, 0, scratchcards.length) + scratchcards.length;
    } catch (error) {
